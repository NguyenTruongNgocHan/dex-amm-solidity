import { Activity, ArrowDownUp, Droplets, Gift, Layers, LogOut } from "lucide-react";
  ...
  return (
    <SurfaceCard className="p-5" hover>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        <Stat label="Total" value={events.length} icon={<Activity size={15} />} tone="neutral" />
        <Stat label="Swaps" value={swaps} icon={<ArrowDownUp size={15} />} tone="blue" />
        <Stat label="Adds" value={adds} icon={<Droplets size={15} />} tone="success" />
        <Stat label="Removes" value={removes} icon={<LogOut size={15} />} tone="danger" />
        <Stat label="Stakes" value={stakes} icon={<Layers size={15} />} tone="violet" />
        <Stat label="Claims" value={claims} icon={<Gift size={15} />} tone="pink" />
      </div>
    </SurfaceCard>
  );
}
